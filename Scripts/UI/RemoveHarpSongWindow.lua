removeHarpSongWindow = class(Turbine.UI.Lotro.GoldWindow);

-- Constructor --
function removeHarpSongWindow:Constructor()
    Turbine.UI.Lotro.GoldWindow.Constructor(self);
	self:SetText("Remove Harp Song");
	...
	self.deleteSongButton.Click = function( sender, args)
		...
		Scripts.harpSongLibrary[self.songSelect:GetValue()] = nil;
...
function removeHarpSongWindow:updateGroupList()
	self.songSelect:ClearList();

	if Scripts.harpSongLibrary ~= {} then

		Scripts.sortedSongLibrary = {};
        for id in pairs(Scripts.harpSongLibrary) do table.insert(Scripts.sortedSongLibrary, id); end
...
-- Set Visibility --
function removeHarpSongWindow:SetVisibility(state)
	if (state == true) then
...
SongManager.UI.removeHarpSongWindow = removeHarpSongWindow();