removeLuteSongWindow = class(Turbine.UI.Lotro.GoldWindow);

-- Constructor --
function removeLuteSongWindow:Constructor()
    Turbine.UI.Lotro.GoldWindow.Constructor(self);
	self:SetText("Remove Lute Song");
	...
	self.deleteSongButton.Click = function( sender, args)
		...
		Scripts.luteSongLibrary[self.songSelect:GetValue()] = nil;
...
function removeLuteSongWindow:updateGroupList()
	self.songSelect:ClearList();

	if Scripts.luteSongLibrary ~= {} then

		Scripts.sortedSongLibrary = {};
        for id in pairs(Scripts.luteSongLibrary) do table.insert(Scripts.sortedSongLibrary, id); end
...
-- Set Visibility --
function removeLuteSongWindow:SetVisibility(state)
	if (state == true) then
...
SongManager.UI.removeLuteSongWindow = removeLuteSongWindow();