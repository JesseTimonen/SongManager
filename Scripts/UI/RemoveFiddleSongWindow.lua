removeFiddleSongWindow = class(Turbine.UI.Lotro.GoldWindow);

-- Constructor --
function removeFiddleSongWindow:Constructor()
    Turbine.UI.Lotro.GoldWindow.Constructor(self);
	self:SetText("Remove Fiddle Song");
	...
	self.deleteSongButton.Click = function( sender, args)
		...
		Scripts.fiddleSongLibrary[self.songSelect:GetValue()] = nil;
...
function removeFiddleSongWindow:updateGroupList()
	self.songSelect:ClearList();

	if Scripts.fiddleSongLibrary ~= {} then

		Scripts.sortedSongLibrary = {};
        for id in pairs(Scripts.fiddleSongLibrary) do table.insert(Scripts.sortedSongLibrary, id); end
...
-- Set Visibility --
function removeFiddleSongWindow:SetVisibility(state)
	if (state == true) then
...
SongManager.UI.removeFiddleSongWindow = removeFiddleSongWindow();